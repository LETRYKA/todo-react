...
const Footer = (props) => {
    ...
    const clearCompletedTasks = () => {
        const updatedTodos = todo.filter((task) => task.status !== 'DONE');
    ...
    return (
        <div style={{ width: '100%' }} className="center">
            <div style={{ width: '80%' }}>
                <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', marginTop: '20px  ' }} className='row'>
                    <p style={{ marginTop: '20px', fontSize: '14px', color: 'var(--gray-color)' }}>
                        {todo.filter(task => task.status === 'DONE').length} of {todo.length} task completed
                    </p>
                    <p style={{ marginTop: '20px', fontSize: '14px', color: 'var(--war)', cursor: 'pointer' }} onClick={clearCompletedTasks}>
                        Clear completed tasks.
                    </p>
                </div>
                <p style={{ marginTop: '20px', fontSize: '14px', color: 'var(--gray-color)' }}>
                    {todo.filter(task => task.status === 'DONE').length} of {todo.filter(task => task.status !== 'LOG').length} task completed
                </p>
